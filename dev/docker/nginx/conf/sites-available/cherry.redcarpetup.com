upstream cherry_app {
  server unix:///var/run/app/cherry_web.sock; 
}
...
upstream cherry_api{
  server unix:///var/run/app/cherry_api.sock;
}

upstream cherry_api2{
  server unix:///var/run/app/cherry_api2.sock;
}

server {
listen 80;
server_name apicherry.redcarpetup.com;
 ...
 server {
 listen 443 ;
 server_name cherry.redcarpetup.com;
  ...
  #      access_log   off;
   ...
   #     deny all;
	...
	proxy_set_header X-Forwarded-Proto $scheme;
        proxy_pass http://cherry_app; # match the name of upstream directive which is defined above
      }
      ...
      location ~ ^/generate_credit_score(/.*|$) {
        return 301  /generate_credit_score/ ;
      }
	...
	proxy_set_header   SSL_PROTOCOL $ssl_protocol;
        ...
        proxy_set_header   SSL_SERVER_S_DN $ssl_client_s_dn;
	proxy_set_header X-Forwarded-Proto $scheme;
        proxy_pass http://cherry_api; # match the name of upstream directive which is defined above
      }
...
location  / {
	...
	proxy_set_header X-Forwarded-Proto $scheme;
        ...
        proxy_set_header   SSL_CLIENT_VERIFY $ssl_client_verify;
        proxy_set_header   SSL_SERVER_S_DN $ssl_client_s_dn;
        proxy_pass http://cherry_api2; # match the name of upstream directive which is defined above
      }
...
server {
listen 80;
server_name api2cherry.redcarpetup.com;
